...
namespace position//命名空间，申明作用域
{
    ...
    //写入预设轨迹或者读取轨迹
    void PositionControl::trajectoryCallback(const moveit_msgs::ExecuteTrajectoryActionGoal::ConstPtr& msg) 
    {
        // 获取关节轨迹
        const trajectory_msgs::JointTrajectory& joint_trajectory = msg->goal.trajectory.joint_trajectory;
        if (joint_trajectory.points.empty()) 
        {
            ROS_WARN("No points in the trajectory message.");
            return;
        }
        ...
        for (const auto& point : joint_trajectory.points) 
        {
            ...
            {
                point_positions.push_back(position);
            }
            positions.push_back(point_positions);
        }
...
{
    // 初始化ROS节点
    ros::init(argc, argv, "GetMoveitTrajectory");
    // 创建节点句柄
    ros::NodeHandle nh;
    ...
    //写入预设轨迹或者读取轨迹
    ros::Subscriber sub = nh.subscribe("execute_trajectory/goal", 100, &position::PositionControl::trajectoryCallback,&positionControl);
    ROS_INFO("Node initialized and subscribed to execute_trajectory/goal");
